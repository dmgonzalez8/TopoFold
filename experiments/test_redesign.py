...
def get_valid_indices(names, dataset_indices):
    valid_indices = []
    missing_names = []
    for name in names:
        try:
            index = dataset_indices[name]
            valid_indices.append(index)
        except KeyError:
            missing_names.append(name)
            # print(f"Warning: {name} not found in dataset and will be skipped.")
    return valid_indices, missing_names
...
# error check
valid_indices, missing_names = get_valid_indices(test_names, dataset_indices)
#
...
with torch.no_grad():
    ...
    for ix, protein in enumerate(test_set):
        ...
        X, S, mask, lengths, seqs = featurize(batch_clones, device)

        log_probs = model(X, S, lengths, mask, seqs)
        ...
        with open(ali_file, 'w') as f:
            ...
            for temp in temperatures:
                for j in range(NUM_BATCHES):
                    S_sample = model.sample(X, lengths, seqs, mask, temperature=temp)

                    # Compute scores
                    log_probs = model(X, S_sample, lengths, mask, seqs)
