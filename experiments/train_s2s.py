...
for e in range(args.epochs):
    ...
    for train_i, batch in enumerate(loader_train):
        ...
        # Get a batch
        X, S, mask, lengths, sequences = featurize(batch, device, shuffle_fraction=args.shuffle)
        elapsed_featurize = time.time() - start_batch

        optimizer.zero_grad()
        log_probs = model(X, S, lengths, mask, sequences)
...
with torch.no_grad():
    ...
    for _, batch in enumerate(loader_test):
        X, S, mask, lengths, sequences = featurize(batch, device)
        log_probs = model(X, S, lengths, mask, sequences)
        loss, loss_av = loss_nll(S, log_probs, mask)
        # Accumulate
