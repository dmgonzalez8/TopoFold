...
class Struct2Seq(nn.Module):
    ...
    def forward(self, X, S, L, mask, sequence):
        """ Graph-conditioned sequence model """

        # Prepare node and edge embeddings
        V, E, E_idx = self.features(X, L, mask, sequence)
        h_V = self.W_v(V)
        h_E = self.W_e(E)
    ...
    def sample(self, X, L, seqs, mask=None, temperature=1.0,):
        """ Autoregressive decoding of a model """
         # Prepare node and edge embeddings
        V, E, E_idx = self.features(X, L, mask, seqs)
        h_V = self.W_v(V)
        h_E = self.W_e(E)
