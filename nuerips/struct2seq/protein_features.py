...
class ProteinFeatures(nn.Module):
    def __init__(self, edge_features, node_features, num_positional_embeddings=16,
        ...
        self.charge_map = {'K': 1, 'R': 1, 'H': 1, 'D': -1, 'E': -1}  # Basic charged amino acids
    ...
    def _charge_interactions(self, sequences, E_idx, mask):
        ...
        # Gather charges based on edges
        charges_i = gather_nodes(charges.unsqueeze(-1), E_idx)
        charges_j = gather_nodes(charges.unsqueeze(-1), E_idx)
    ...
    def _dist(self, X, mask, eps=1E-6):
        ...
        actual_top_k = min(self.top_k, D_adjust.size(-1))

        D_neighbors, E_idx = torch.topk(D_adjust, actual_top_k, dim=-1, largest=False)
    ...
    def forward(self, X, L, mask, sequences=None):
